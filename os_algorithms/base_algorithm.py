...
class BaseAlgorithm:
    ...
    def __call__(self, fullColumnData, predict=False):
        ...
        if predict:
            data = fullColumnData[["lon", "lat", "date","region_id", "distance", 'depth']+self.input_names()+columnErrorNames]; #Subset to select only the columns used
        else: #using output variable, date etc.
            data = fullColumnData[["date", "lon", "lat","region_id", "distance", "depth"]+self.input_names()+columnErrorNames+[self.output_name()]]; #Subset to select only the columns used
